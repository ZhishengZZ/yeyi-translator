...
- Per-site rule engine (include/exclude/force-block/force-inline selectors, min-length gates, injected CSS) ported from Read Frog's `utils/host/dom/filter.ts` and `utils/site-rules/*`; the built-in wikipedia/shopee rules are adapted from its `site-rules/built-in/rules.json`; drop-cap (large initial floating letter) handling from `filter.ts`. / 站点规则引擎、内置 wikipedia/shopee 规则与首字下沉判定移植自 Read Frog 上述文件。
